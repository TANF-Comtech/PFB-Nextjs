PeopleForBikes site is a [Next.js](https://nextjs.org/) project using [Tailwind](https://tailwindcss.com/) (and [styled-components](https://styled-components.com/docs)) for CSS, [Apollo](https://www.apollographql.com/docs/react/) for component-level data-fetching, and [Prismic.io](https://peopleforbikes.prismic.io/) as our headless CMS.
...
- [Tailwind](https://tailwindcss.com/), our current styling tool
- [styled-components](https://styled-components.com/docs), our legacy styling tool
...
The project is set up to support writing both JavaScript and TypeScript. To use TypeScript, simply switch to using the `.tsx` extension for pages/components and the `.ts` extension for any files not containing JSX markup.

#### Imports

[Module path aliases](https://nextjs.org/docs/advanced-features/module-path-aliases) are configured to allow for using absolute paths instead of relative paths.

For example if you are editing a deeply nested page at `/pages/deeply/nested/route/index.tsx` and you want to import a utility function from `/utils` you would normally have to write the full relative path:

```
import { someFunction } from '../../../../../utils'
```

However with module path aliases in we can import the same function using `~/` to represent an absolute path from the root directory:

```
import { someFunction } from '~/utils'
```

The configuration for the root indicator is set in `jsconfig.json` and `tsconfig.json` for JavaScript and TypeScript respectively.
...
- `/components/` contains reusable React components that are shared between multiple pages and components. Components that are consumed only once are considered "template partials" and are periodically moved into their consuming page/component file to reserve this directory for more general purpose components.
...
- inside of the function, you want to `await` the custom `getPrismicData()` function, which is where you will call your named query (and that query will typically accept the params from the function)

Example page-based query (from `/lib/queries/careers.js`):

```js
import { getPrismicData, API_LOCALE } from '../api';
...
New components should be styled with [Tailwind](https://tailwindcss.com/), which has been configured to use the

This site also makes heavy use of [`styled-components`](https://styled-components.com/docs/basics) for styling. This is component-scoped, CSS-in-JS solution that creates a template literal component for each HTML element. The components usually sit atop a larger component with a render method:
...
Global styles can be added to `~/styles/globals.css`.
...
- Use functional components and hooks where you can. Classes are ok, but only if you really need legacy lifecycle methods or error boundaries.
- Make components as small as possible. Don't go crazy with this but use it as a general guiding principle.
- Don't wrap components in unnecessary wrapper `<div>` pairs, use [React Fragment short syntax](https://reactjs.org/docs/fragments.html#short-syntax) instead: `<></>`
...
const ExampleComponent = () => {
  return (
    <>
      <Image alt="Take Action Banner" src={Banner} quality={80} width={1600} height={800} />
    </>
  );
};
```
        ...
        <>
          ...
          {slice.fields &&
            slice.fields.map((item) => {
              return contentBlob.push(item.sub_pillar, item.sub_pillar_summary);
            })}
        </>
      );
    });
