...
import { getEBikesPages, 
         getSingleEBikesPage } from '../../lib/queries/electric-bikes'
...
export default function EBikesPage({ page, preview }) {
  ...
  return (
    <>
      <SiteMeta
        desc={ electric_bikes.main_content ? ( `${ electric_bikes.main_content[0].text.substring(0,180) } ... ` ) : ( meta.desc ) }
        title={ electric_bikes.title ? ( `${ electric_bikes.title[0].text } - Electric Bikes | People for Bikes` ) : ( meta.title ) }
        imgHeight={ meta.imgHeight }
        imgSrc={ meta.imgSrc }
        imgWidth={ meta.imgWidth }
        path={ electric_bikes ? ( `https://www.peopleforbikes.org/electric-bikes/${electric_bikes._meta.uid}` ) : ( meta.path ) }
      />    
      <Wrapper 
