...
import { getStats, 
         getSingleStatsPage } from '../../lib/queries/statistics'
...
export default function MembersPage({ page, preview }) {
  ...
  return (
    <>
      <SiteMeta
        desc={ statistic_page.main_content ? ( `${ statistic_page.main_content[0].text.substring(0,180) } ... ` ) : ( meta.desc ) }
        title={ statistic_page.title ? ( `${ statistic_page.title[0].text } Statistics | People for Bikes` ) : ( meta.title ) }
        imgHeight={ meta.imgHeight }
        imgSrc={ meta.imgSrc }
        imgWidth={ meta.imgWidth }
        path={ statistic_page ? ( `https://www.peopleforbikes.org/statistics/${statistic_page._meta.uid}` ) : ( meta.path ) }
      />    
      <Wrapper 
        postPath="/research"
        postTitle="Research"
