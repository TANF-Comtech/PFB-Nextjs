 ...
 */
 export function grantsDate(cycleDate) {
   ...
   if ( splitDate[0].toLowerCase() === 'spring') { return `${splitDate[1]}-05-01` } 
   else if ( splitDate[0].toLowerCase() === 'summer') { return `${splitDate[1]}-08-01` }
   else if ( splitDate[0].toLowerCase() === 'fall') { return `${splitDate[1]}-11-01` }
   else if ( splitDate[0].toLowerCase() === 'winter') { return `${splitDate[1]}-02-01` }
   else if ( typeof(splitDate[0]) === 'number' ) { return `${splitDate[0]}-01-01` }
   else return null
 } 
  ...
  if ( payload.length > 1 ) {
    payload.map( item => {
      ...
      formattedPayload.push({
        ...
        topics: null,
        locations: item.node.location1 ? 
                    [ item.node.location1.location[0].text ] : null 
      })
